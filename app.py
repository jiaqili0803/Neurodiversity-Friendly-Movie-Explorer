...
# Visual encodings (colors + shapes)
...
# Map each genre to a unique color (cycle only if more than 12 genres)
genre_color_map = {
    g: user_palette[i % len(user_palette)] for i, g in enumerate(unique_genres)
}

# Marker shapes per genre
shape_list = [
    "circle",
    "square",
    "diamond",
...
genre_shape_map = {
    g: shape_list[i % len(shape_list)] for i, g in enumerate(unique_genres)
}
...
def make_network_figure(mode="simple", focus_id=None, search_id=None):
    ...
    for _, row in e.iterrows():
        ...
        is_focus_edge = (
            ...
            and (row["source"] == focus_id or row["target"] == focus_id)
        )
        if is_focus_edge:
    ...
    # ----- Legend traces: color + shape per genre -----
    legend_traces = []
    ...
    traces = [other_edges_trace, focus_edges_trace, node_trace] + legend_traces
    ...
    children=[
        ...
        html.Div(
            [
                ...
                html.Div(
                    [
                        # LEFT COLUMN: visual legend explanation (no explicit color list now)
                        html.Div(
                            [
                                html.Div(
                                    "Visual legend",
                                ...
                                html.Ul(
                                    [
                                        ...
                                        html.Li(
                                            "Color + shape combinations for each genre are shown in the legend at the top of the network view."
                                        ),
                                    ],
                                    style={
                                        "paddingLeft": "18px",
        ...
        # Controls row (Display mode + search) – with extra vertical spacing
        html.Div(
            [
                html.Div(
                    [
...
# Callback: hover + click + search + reset
...
def update(
    ...
    if prop_id == "reset-view":
        ...
        if locked_focus_id is not None:
            ...
            center_id = None  # no recenter on hover
        else:
